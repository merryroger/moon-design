...
const sourceDir = path.join(workDir, 'packages', 'icons', 'raw', 'imported');
...
const replacements = [
	...
	{
		pattern: new RegExp("<section className=\"mt-8\">[\\n\\c\\r\\t\\s]+?<SubHeader>.+<\/section>", 's'),
		replace(data) {
...
let dataSet = fs.readdirSync(sourceDir).reduce((result, item) => {
		let match = Array.from(item.matchAll(pattern));
		let fileName = match[0][0];
		let blockName = match[0][1];
